...
The existing FairwayOS design exports inform the green/gold palette, Playfair Display and Inter typography, editorial layout, and supplied imagery. The landing-page backdrop remains illustrative design artwork; course cards use explicitly labeled generated landscape archetypes, with information-card fallbacks for courses without imagery.
...
Course imagery uses a discriminated `CoursePhoto` contract: `illustration` is explicitly labeled AI-generated / not an actual course photo, while `photograph` requires credit, source, and license metadata. Six generated landscapes now cover the curated courses. The scenes evoke broad terrain categories without reproducing actual layouts or using existing course photos as references. See `docs/course-landscapes.md` and the machine-readable `docs/course-landscapes.json` for every saved asset and exact generation prompt.

The built-in image tool returned 1536×1024 originals. Static WebP variants at 480, 960, and 1536px are selected with srcset/sizes; images are never upscaled during asset preparation. Cards and heroes retain intrinsic dimensions, loading priority, and an error fallback. Run `python scripts/prepare_course_photo.py INPUT SLUG --collection illustrations` with Pillow installed for generated images, or omit the collection for licensed photos. Originals are preserved in ignored `.build-assets/illustrations/`; served assets are checked in under `public/illustrations/`. The prior licensed Fossil Trace photograph remains as an unused asset; provenance: James St. John, 2007-11-01, CC BY 2.0, https://commons.wikimedia.org/wiki/File:Fossil_Trace_Golf_Course_(Golden,_Colorado,_USA)_2.jpg . Original supplied course PNGs are also retained unused. The landing-page scenic backdrop remains illustrative.
